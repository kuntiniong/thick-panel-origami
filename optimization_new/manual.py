...
class ThickPanelManualFramework(ThickPanelDesignFramework):
    ...
    def optimize(
        ...
        if manual_offsets.size in (self.num_creases, self.num_independent):
            if manual_offsets.size == self.num_creases:
                candidate = self._physical_offsets_to_optimizer_vars(manual_offsets)
            else:
                candidate = np.asarray(manual_offsets, dtype=float)
        else:
            raise ValueError(
