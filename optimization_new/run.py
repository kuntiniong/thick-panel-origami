...
_OPT_NEW_DIR = os.path.dirname(os.path.abspath(__file__))
_PROJECT_ROOT = os.path.dirname(_OPT_NEW_DIR)
if _PROJECT_ROOT not in sys.path:
    sys.path.insert(0, _PROJECT_ROOT)

from optimization_new.framework import (
    AlgorithmSpec,
...
def _resolve_config_file() -> str:
    ...
    raise FileNotFoundError(
        "No optimization config found. Create optimization_new/config.yml "
        "from optimization_new/config.example.yml"
    )
...
def _framework_kwargs(config: Dict[str, Any], json_path: str) -> Dict[str, Any]:
    ...
    kwargs = {
        ...
        "discrete_step": framework_cfg.get("discrete_step", 0.1),
        "max_offset": framework_cfg.get("max_offset", 20.0),
...
def _load_bo_framework_class():
    from optimization_new.algorithms.bo import ThickPanelBOFramework
...
def _load_manual_framework_class():
    from optimization_new.manual import ThickPanelManualFramework

    return ThickPanelManualFramework


def _load_de_framework_class():
    from optimization_new.algorithms.de import ThickPanelDEFramework
...
def _cma_es_optimize_kwargs(config: Dict[str, Any], common: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {**common, "sigma_init": cma_cfg.get("sigma_init", 1.0)}
...
def _cma_es_margin_optimize_kwargs(config: Dict[str, Any], common: Dict[str, Any]) -> Dict[str, Any]:
    ...
    kwargs = {**common, "sigma_init": cma_cfg.get("sigma_init", 1.0)}
