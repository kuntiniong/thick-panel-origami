...
class ThickPanelDesignFramework:
    ...
    def simulator_origami_name(self) -> str:
        """Name passed to OrigamiSimulator; controls physResult/<prefix>-* folder."""
    ...
    def _shared_simulator_name(self) -> str:
        """Result folder name shared by all workers in one optimization run."""
        stem = os.path.basename(self.json_path).replace(".json", "")
        stem = f"{stem}_batch_{self.batch_size}"
        if self.result_prefix:
            return f"{self.result_prefix}-{stem}"
        return stem
